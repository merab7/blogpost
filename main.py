...
class New_blog_f(FlaskForm):
    ...
    img_url = StringField('URL for background', validators=[DataRequired()])
...
@app.route('/')
def get_all_posts():
    query_all = db.session.execute(db.select(BlogPost).order_by(BlogPost.id)).scalars().all()
    posts = [x for x in query_all]
